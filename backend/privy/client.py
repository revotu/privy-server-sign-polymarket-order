...
from privy.auth_signature import canonicalize_payload, compute_authorization_signature
...
class PrivyWalletClient:
    ...
    def authenticate_wallet(self, user_jwt: str) -> str:
        ...
        流程说明 / Flow:
            ...
            同时作为 privy-authorization-jwt 头和 ECDSA 签名密钥执行写操作。
            ...
            authorization_key both as privy-authorization-jwt header AND as the ECDSA signing key.
        ...
        Returns:
            authorization_key 字符串（PKCS8 DER base64 格式的 P256 私钥），
            同时用作 privy-authorization-jwt 头和 ECDSA 签名密钥。
            authorization_key string (P256 private key in PKCS8 DER base64 format),
            used both as privy-authorization-jwt header and as the ECDSA signing key.
    ...
    def add_signer_to_wallet(
        ...
        # authorization_key 是 Privy TEE 为本次会话生成的 P256 私钥（PKCS8 DER base64）
        # authorization_key is a P256 private key (PKCS8 DER base64) generated by Privy TEE for this session
        authorization_key = self.authenticate_wallet(user_jwt)

        url = self._get_wallet_url(wallet_id)

        # 请求体：添加 Key Quorum 作为 signer（不含 policy_ids，此字段在该接口不被接受）
        # Request body: add Key Quorum as signer (no policy_ids, that field is rejected by this endpoint)
        body = {
            "additional_signers": [
        ...
        # Step 2: 用 authorization_key 作为签名密钥计算 privy-authorization-signature
        # authorization_key 既作为 privy-authorization-jwt 头，也作为 ECDSA 签名密钥
        # Step 2: Use authorization_key as signing key to compute privy-authorization-signature
        # authorization_key serves both as privy-authorization-jwt header AND as the ECDSA signing key
        session_private_key = serialization.load_der_private_key(
            base64.b64decode(authorization_key), password=None
        )
        payload = {
            "version": 1,
            "method": "PATCH",
        ...
        sig_bytes = session_private_key.sign(
            canonicalize_payload(payload).encode("utf-8"),
            ec.ECDSA(hashes.SHA256()),
        )
        authorization_signature = base64.b64encode(sig_bytes).decode()
