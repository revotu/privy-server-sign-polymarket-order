...
def compute_user_authorization_signature(
    url: str,
    body: dict[str, Any],
    app_id: str,
    user_jwt: str,
    ...
    """
    使用用户 JWT 计算授权签名（用于用户作为 wallet owner 时的操作）。
    Computes authorization signature using user JWT (for operations where user is wallet owner).

    当用户的钱包需要用户本人授权时（如修改 wallet 配置、添加 signer），
    需要传入用户的 JWT 而不是服务端的授权密钥。
    When user wallet operations require user authorization (e.g., modifying wallet config,
    adding signers), pass user JWT instead of server authorization key.

    注意：此函数通过在签名 payload 中包含用户 JWT header 来实现用户身份证明。
    Note: This function includes the user JWT header in the signature payload to prove user identity.

    Args:
        url: 完整的请求 URL / Full request URL
        body: 请求体字典 / Request body dictionary
        app_id: Privy App ID
        user_jwt: 用户的 Privy access token（JWT）/ User's Privy access token (JWT)
        method: HTTP 方法 / HTTP method

    Returns:
        base64 编码的签名，此签名将在 Privy API 内部用用户的 signing key 验证
        Base64-encoded signature, verified internally by Privy using user's signing key
    """
    # 用户签名的 payload 在 headers 中包含 privy-authorization-jwt
    # User signature payload includes privy-authorization-jwt in headers
    payload = {
        "version": 1,
        "method": "PATCH",
        "url": url,
    ...
    # 用户签名通过 Privy SDK 的 user signing key 机制处理
    # 直接返回规范化后的 payload 供外部调用方通过 Privy API 请求用户签名
    # User signing is handled by Privy SDK's user signing key mechanism
    # Return the canonicalized payload for the caller to request user signature via Privy API
