...
async def bind_signer(request: BindSignerRequest):
    ...
    绑定后服务端即可无弹窗代替用户签名（sign_typed_data / sign_message）。
    ...
    流程 / Flow:
        ...
        3. 后端用 user_jwt 作为授权凭证调用 Privy PATCH /wallets/{wallet_id}
        4. Privy 验证 JWT 合法后将 Key Quorum 添加为该 wallet 的 additional_signer
        5. 后续所有签名操作无需再传 user_jwt

    Args:
        request.wallet_id: 用户的 Privy wallet ID
        request.user_jwt:  用户的 Privy access token（Flutter SDK 返回）
    """
    try:
